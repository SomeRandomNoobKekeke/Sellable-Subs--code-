...
namespace SellableSubs
{
  public partial class Plugin : IAssemblyPlugin
  {
    ...
    public List<SubmarineSelection> screens = new List<SubmarineSelection>();
    ...
    public void patchClient()
    {
      harmony.Patch(
        ...
        prefix: new HarmonyMethod(typeof(Plugin).GetMethod("SubmarineSelection_CreateGUI_Replace"))
      );

      harmony.Patch(
        original: typeof(SubmarineSelection).GetMethod("UpdateSubmarines", AccessTools.all),
        prefix: new HarmonyMethod(typeof(Plugin).GetMethod("SubmarineSelection_UpdateSubmarines_Replace"))
      );

      harmony.Patch(
        original: typeof(SubmarineSelection).GetMethod("RefreshSubmarineDisplay", AccessTools.all),
        prefix: new HarmonyMethod(typeof(Plugin).GetMethod("SubmarineSelection_RefreshSubmarineDisplay_Replace"))
      );

      harmony.Patch(
        original: typeof(SubmarineSelection).GetMethod("ShowBuyPrompt", AccessTools.all),
        prefix: new HarmonyMethod(typeof(Plugin).GetMethod("SubmarineSelection_ShowBuyPrompt_Replace"))
      );

      harmony.Patch(
        original: typeof(VotingInterface).GetMethod("SetSubmarineVotingText", AccessTools.all),
        prefix: new HarmonyMethod(typeof(Plugin).GetMethod("VotingInterface_SetSubmarineVotingText_Replace"))
      );
      ...
      harmony.Patch(
        ...
        prefix: new HarmonyMethod(typeof(Plugin).GetMethod("SubmarineSelection_SelectSubmarine_Replace"))
      );

      harmony.Patch(
        original: typeof(SubmarineSelection).GetConstructors()[0],
        postfix: new HarmonyMethod(typeof(Plugin).GetMethod("SubmarineSelection_Constructor_Postfix"))
      );
    }
